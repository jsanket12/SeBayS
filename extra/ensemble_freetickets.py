...
from OoD_expts_pytorch import CIFAR10_M, CIFAR100_M, make_ood_dataset, eval_metrics
sns.set()
...
def evaluate_ood(model,device,test_loader):
    ood_labels_G = []
    ood_scores_G = []
    model.eval()
...
def adv_test(model, device, test_loader, clip_min, clip_max, epsilon):
    model.eval()
    # Accuracy counter
    ...
    for i in range(args.iters):
        ...
        if args.resume:
                ...
                if 'disagreement' in args.mode:
                    ...
                    plt.savefig("./plots/" + "EDST_RN32_CF10_dense.pdf")
                ###############################################################################
                #                          predict                                            #
                ###############################################################################
                ...
                if 'adv' in args.mode:
                    ...
                    for file in range(0, len(model_files)):
                        ...
                        indi_acc, indi_acc_adv, indi_loss, indi_loss_adv = adv_test(model, device, test_loader,clip_min,clip_max,epsilon=args.epsilon)
                        acc.append(indi_acc)
                        nll_loss.append(indi_loss)
                ...
                if 'ood' in args.mode:
                    ...
                    for file in range(0, len(model_files)):
                        ...
                        ood_labels_G, ood_scores_G = evaluate_ood(model, device, test_loader)
