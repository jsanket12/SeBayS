    ...
    for i in range(args.iters):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if args.use_bnn:
                if not args.lr_std_diff:
                    if not args.steps_diff:
                        save_dir = 'results_indi' + '/' + str(args.model) + '/' + str(args.data) +  '/' + 'wd_' + str(args.weight_decay) +  '/' + 'density_' + str(
                            args.density) + '/' + 'BayS' + '/' + str(args.sparsity_distribution) + '/sig_' + str(args.prior_std) +  '/anneal_' + str(args.kl_anneal) + '/lr_std_same/' + 'seed=%d' % (args.seed)
                    else:
                        save_dir = 'results_indi' + '/' + str(args.model) + '/' + str(args.data) +  '/' + 'wd_' + str(args.weight_decay) +  '/' + 'density_' + str(
                            args.density) + '/' + 'BayS' + '/' + str(args.sparsity_distribution) + '/sig_' + str(args.prior_std) +  '/anneal_' + str(args.kl_anneal) + '/steps_diff/lr_std_same/' + 'seed=%d' % (args.seed)
                else:
                    if not args.steps_diff:
                        save_dir = 'results_indi' + '/' + str(args.model) + '/' + str(args.data) +  '/' + 'wd_' + str(args.weight_decay) +  '/' + 'density_' + str(
                            args.density) + '/' + 'BayS' + '/' + str(args.sparsity_distribution) + '/sig_' + str(args.prior_std) +  '/anneal_' + str(args.kl_anneal) + '/lr_std_diff/' + 'seed=%d' % (args.seed)
                    else:
                        save_dir = 'results_indi' + '/' + str(args.model) + '/' + str(args.data) +  '/' + 'wd_' + str(args.weight_decay) +  '/' + 'density_' + str(
                            args.density) + '/' + 'BayS' + '/' + str(args.sparsity_distribution) + '/sig_' + str(args.prior_std) +  '/anneal_' + str(args.kl_anneal) + '/steps_diff/lr_std_diff/' + 'seed=%d' % (args.seed)

            else:
                save_dir = 'results' + '/' + str(args.model) + '/' + str(args.data) +  '/' + 'wd_' + str(args.weight_decay) + '/' + 'density_' + str(
                        args.density) + '/' + 'DST' + '/Uniform/' + 'seed=%d' % (args.seed)
            ...
            if not args.steps_diff:
                ...
                # if epoch > int(3*args.epochs/5)-4:
                save_checkpoint({
                        'epoch': epoch,
                        'state_dict': model.state_dict(),
                    }, filename=os.path.join(save_dir, 'model_epoch_%d.pth' % epoch))
            else:
