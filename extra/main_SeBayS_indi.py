...
def main():
    ...
    for i in range(args.iters):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if args.use_bnn:
                if not args.lr_std_diff:
                    save_dir = 'results_indi' + '/' + str(args.model) + '/' + str(args.data) +  '/' + 'wd_' + str(args.weight_decay) + '/random' + '/seed' + str(args.seed) + '/density_' + str(
                            args.density) + '/' + 'SeBayS' + '/' + str(args.sparsity_distribution) + '/sig_' + str(args.prior_std) + '/anneal_' + str(args.kl_anneal) + '/lr_std_same/' +  'M=' + str(args.model_num)
                else:
                    save_dir = 'results_indi' + '/' + str(args.model) + '/' + str(args.data) +  '/' + 'wd_' + str(args.weight_decay) + '/random' + '/seed' + str(args.seed) + '/density_' + str(
                            args.density) + '/' + 'SeBayS' + '/' + str(args.sparsity_distribution) + '/sig_' + str(args.prior_std) + '/anneal_' + str(args.kl_anneal) + '/lr_std_diff/' +  'M=' + str(args.model_num)
            else:
                save_dir = 'results_indi' + '/' + str(args.model) + '/' + str(args.data) +  '/' + 'wd_' + str(args.weight_decay) + '/random' + '/seed' + str(args.seed) + '/density_' + str(
                        args.density) + '/' + 'EDST' + '/Uniform/' +  'M=' + str(args.model_num)
            ...
            if epoch > args.epochs_explo - 4:
                subnetwork_dir = save_dir+'/Subnet_'+str((epoch - args.epochs_explo - 1)//args.individual_epoch + 1)
                if not os.path.exists(subnetwork_dir): os.makedirs(subnetwork_dir)
                save_checkpoint({
                        'epoch': epoch,
                        'state_dict': model.state_dict(),
                    }, filename=os.path.join(subnetwork_dir, 'model_epoch_%d.pth' % epoch))
