...
class MLP_MNIST(nn.Module):
    ...
    def forward(self, x):
        x0 = F.relu(self.fc1(x.view(-1, 28*28)))
        x1 = F.relu(self.fc2(x0))
        return F.log_softmax(self.fc3(x1), dim=1)
...
#     def forward(self, x):
#         residual = x
#         out = self.relu(self.bn1(self.conv1(x))) #F.silu(self.bn1(self.conv1(x)))
#         out = self.relu(self.bn2(self.conv2(out))) #F.silu(self.bn2(self.conv2(out)))
#         out = self.bn3(self.conv3(out))
#         if self.downsample is not None:
#             residual = self.downsample(x)
#         out += residual
#         out = self.relu(out) #F.silu(out)
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        x = self.relu(self.bn1(self.conv1(x)))    # 32x32
