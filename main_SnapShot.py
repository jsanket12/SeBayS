...
from se_models.se_wide_resnet import SE_WideResNet
...
models = {}
models['wrn-28-10'] = (SE_WideResNet, [28, 10, 10, 0.0])
...
def proposed_lr(initial_lr, iteration, epoch_per_cycle):
    """
    Cosine annealing learning rate schedule within a cycle.
    """
...
def main():
    ...
    parser.add_argument('--cycles', type=int, default=5, metavar='N',
                        help='number of cycles (snapshots) to take (default: 3)')
    ...
    if args.optimizer == 'sgd':
        optimizer = optim.SGD(model.parameters(),lr=args.initial_lr,momentum=args.momentum,weight_decay=args.l2)
    elif args.optimizer == 'adam':
        optimizer = optim.Adam(model.parameters(),lr=args.initial_lr,weight_decay=args.l2)
    else:
    ...
    for cycle in range(args.cycles):
        for epoch in range(epochs_per_cycle):
            print_and_log('Cycle: {0}, Epoch: {1}'.format(cycle+1, epoch+1))

            save_dir = 'results_SnapShot' + '/' + str(args.model) + '/' + str(args.data) + \
                        '/seed_' + str(args.seed) + '/init_lr_' + str(args.initial_lr) + '/cycle_len_' \
                        + str(epochs_per_cycle) +  '/C=' + str(args.cycles)
            ...
            lr = proposed_lr(args.initial_lr, epoch, epochs_per_cycle)
            ...
            print_and_log('Time taken for epoch: {0:.2f} seconds.\n'.format(time.time() - t0))

        save_checkpoint({
                        'epoch': epoch + 1,
                        'state_dict': model.state_dict(),
                        'optimizer': optimizer.state_dict(),
        }, filename=os.path.join(save_dir, 'model_%d.pth' % cycle))
