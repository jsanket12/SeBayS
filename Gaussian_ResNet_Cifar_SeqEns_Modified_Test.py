...
def main():
    ...
    parser.add_argument('--results_path', default='./results_resnet/cifar10/Gauss_SeqEns_Modified/', 
    ...
    parser.add_argument('--corrupt', action='store_true', default=False, help='Freeze sparsity after exploration. Default: True.')  
    ...
    model = Gauss_ResNet_Cifar(depth=args.depth, num_classes=target_dim, sigma_0 = sigma_0).to(device)
    ...
    RESULTS_PATH = args.results_path + 'Perturb=%d/' % (args.perturb_factor) + 'Seed=%d/' % (args.seed)

    MODELS_PATH = RESULTS_PATH + 'Models/'

    df = pd.read_csv(RESULTS_PATH+'results_Gaussian_RN_'+str(args.depth)+'_seed_'+str(args.seed)+
    ...
    if 'predict' in args.mode:
        ...
        for m in range(args.model_num):
            ...
            model.load_state_dict(torch.load(MODELS_PATH + 'Gaussian_RN_'+str(args.depth)+'_seed_'+str(args.seed)+
                                        '_sig0_'+str(args.sigma_0)+'_perturb_'+str(args.perturb_factor)+
                                        '_clip_'+str(args.clip)+'_val_'+str(args.valid_split)+
                                        '_batch_'+ str(args.batch_size)+'_explo_epoch_'+str(args.epochs_explo)+
                                        '_lr_'+str(args.init_lr)+'_total_epochs_'+str(args.total_epochs)+
    ...
    if 'tsne' == args.mode:
        ...
        for epoch in range(2,int(milestone1[2]+1),2):
            if epoch < 151: continue
        ...
        plt.savefig('Gaussian_RN_'+str(args.depth)+'_seed_'+str(args.seed)+'_perturb_'+str(args.perturb_factor)+'_New.png')
        plt.savefig('Gaussian_RN_'+str(args.depth)+'_seed_'+str(args.seed)+'_perturb_'+str(args.perturb_factor)+'_New.pdf')
        # reshape
