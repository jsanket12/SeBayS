...
from data.data_utils import get_cifar10_dataloaders, get_cifar100_dataloaders, get_svhn_dataloaders
...
def train(args, model, device, train_loader, optimizer, epoch, NTrain, pruner):
    ...
    # if args.use_bnn:
    #     return {'train_acc': 100. * correct / float(total_sample), 'train_loss': loss_avg, 'lr': lr_avg, 'loss_kl': loss_kl}
    # else:
    #     return {'train_acc': 100. * correct / float(total_sample), 'train_loss': loss_avg, 'lr': lr_avg}
...
def evaluate(args, model, device, test_loader, is_test_set=False):
    ...
    param_overall = 0.
    non_zero_params_overall = 0.
    flops_overall = 0.
    sparse_flops_overall = 0.
    for module in model.modules():
        if isinstance(module, (StoConv2d)):
            non_zero_params = (module.weight!=0).sum()
            non_zero_params_overall += non_zero_params
            if module.bias is not None:
                layer_params = module.weight.numel() + module.bias.numel()
                param_overall += layer_params
                layer_flops = 2*module.out_channels*(module.in_channels*module.kernel_size[0]*module.kernel_size[1]+1)* \
                ...
                sparse_layer_flops = layer_flops * (non_zero_params/layer_params)
                flops_overall += layer_flops
                sparse_flops_overall += sparse_layer_flops
            else:
                layer_params = module.weight.numel()
                param_overall += layer_params
                layer_flops = 2*module.out_channels*(module.in_channels*module.kernel_size[0]*module.kernel_size[1])* \
                                (np.floor((module.input_size[2]+2*module.padding[0]-module.dilation[0]*(module.kernel_size[0]-1)-1)/module.stride[0]+1))**2/module.groups
                sparse_layer_flops = layer_flops * (non_zero_params/layer_params)
                flops_overall += layer_flops
                sparse_flops_overall += sparse_layer_flops
            # print_and_log('Module: {}, Nonzero Params {:.4E}, All Parameters: {:.4E}, Sparse FLOPs: {:.4E}, FLOPs: {:.4E}'.format(module, non_zero_params, layer_params, sparse_layer_flops, layer_flops))
        elif isinstance(module, (StoLinear)):
            non_zero_params = (module.weight!=0).sum()
            non_zero_params_overall += non_zero_params
            if module.bias is not None:
                layer_params = (module.in_features+1)*module.out_features
                param_overall += layer_params
                layer_flops = 2*(module.in_features+1)*module.out_features
                sparse_layer_flops = layer_flops * (non_zero_params/layer_params)
                flops_overall += layer_flops
                sparse_flops_overall += sparse_layer_flops
            else:
                ...
                sparse_flops_overall += sparse_layer_flops
            # print_and_log('Module: {}, Nonzero Params {:.4E}, All Parameters: {:.4E}, Sparse FLOPs: {:.4E}, FLOPs: {:.4E}'.format(module, non_zero_params, layer_params, sparse_layer_flops, layer_flops))
    print_and_log('Number of nonzero paramaters {:.4E}, Number of parameters: {:.4E}, Number of sparse FLOPs: {:.4E}, ]Number of FLOPs: {:.4E}'.format(non_zero_params_overall, param_overall, sparse_flops_overall, flops_overall))
    print_and_log('Sparsity: {:.4f}, FLOPs ratio: {:.4f}'.format(non_zero_params_overall/param_overall, sparse_flops_overall/flops_overall))
...
def main():
    ...
    print_and_log('\n\n')
    print_and_log('='*80)
    torch.manual_seed(args.ens_seed)
    random.seed(args.ens_seed)
    np.random.seed(args.ens_seed)
    for i in range(args.iters):
        ...
        if args.data == 'cifar10':
            ...
            train_loader, valid_loader, _ = get_svhn_dataloaders(args, args.valid_split, max_threads=args.max_threads)
            outputs = 10
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if args.use_bnn:
                if not args.lr_std_diff:
                    save_dir = 'results' + '/' + str(args.model) + '/' + str(args.data) +  '/' + 'wd_' + str(args.weight_decay) + '/' + 'death_'+ str(args.death_rate) + '_large_death_' + str(
                            args.large_death_rate) + '/random' + '/seed_' + str(args.ens_seed) + '/density_' + str(args.density) + '/' + 'SeBayS' + '/' + str(
                            args.sparsity_distribution) + '/sig_' + str(args.prior_std) + '/anneal_' + str(args.kl_anneal) + '/lr_std_same/' +  'M=' + str(args.model_num)
                else:
                    save_dir = 'results' + '/' + str(args.model) + '/' + str(args.data) +  '/' + 'wd_' + str(args.weight_decay) + '/' + 'death_'+ str(args.death_rate) + '_large_death_' + str(
                            args.large_death_rate) + '/random' + '/seed_' + str(args.ens_seed) + '/density_' + str(args.density) + '/' + 'SeBayS' + '/' + str(
                            args.sparsity_distribution) + '/sig_' + str(args.prior_std) + '/anneal_' + str(args.kl_anneal) + '/lr_std_diff/' +  'M=' + str(args.model_num)
            else:
                save_dir = 'results' + '/' + str(args.model) + '/' + str(args.data) +  '/' + 'wd_' + str(args.weight_decay) + '/' + 'death_'+ str(args.death_rate) + '_large_death_' + str(
                            args.large_death_rate) + '/random' + '/seed_' + str(args.ens_seed) + '/density_' + str(args.density) + '/' + 'EDST' + '/Uniform/' +  'M=' + str(args.model_num)

            if not os.path.exists(save_dir): os.makedirs(save_dir)

            t0 = time.time()

            if epoch in [x - 1 for x in milestone1]: 
                pruner.large_death_flag = True

            # _ = evaluate(args, model, device, valid_loader)
            # break
            ...
            if epoch == args.epochs_explo:
                ...
                best_acc = 0.0
                pruner.alpha = args.large_death_rate
                pruner._rigl_step()
                pruner.rigl_steps += 1
                pruner.alpha = args.death_rate
                pruner.large_death_flag = False
                optimizer.param_groups[0]['lr'] = 0.01
