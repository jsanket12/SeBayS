...
def ensemble_calibration(val_loader, model, args):
    ...
    for file in range(0, len(model_files)):
        ...
        ind_folds_preds_MC = []
        ind_folds_preds_MC.append(y_pred)
        all_folds_preds_MC.append(y_pred)
        for _ in range(args.num_MC_Test-1):
            y_pred_MC, _ = extract_prediction(val_loader, model, args)
            ece_MC = expected_calibration_error(y_true, y_pred_MC)
            nll_MC = F.nll_loss(torch.from_numpy(y_pred_MC).log(), torch.from_numpy(y_true), reduction="mean")
            val_ece.append(ece_MC)
            nll_loss.append(nll_MC)
            ind_folds_preds_MC.append(y_pred_MC)
            all_folds_preds_MC.append(y_pred_MC)
            print('* Individual Model ECE = {}'.format(ece_MC))
            print('* Individual Model NLL = {}'.format(nll_MC))

        individual_ece_mean = np.array(val_ece).mean(axis=0)
        individual_ece_std = np.array(val_ece).std(axis=0)
        individual_nll_mean = np.array(nll_loss).mean(axis=0)
        individual_nll_std = np.array(nll_loss).std(axis=0)
        print(f"Averaged individual model: ECE is {individual_ece_mean} and std is {individual_ece_std}")
        print(f"Averaged individual model: NLL is {individual_nll_mean} and std is {individual_nll_std}")
      

        output_mean_MC = np.mean(np.stack(ind_folds_preds_MC, 0), 0)
        indi_nll_MC = F.nll_loss(torch.from_numpy(output_mean_MC).log(), torch.from_numpy(y_true), reduction='mean').item()
        indi_ece_MC = expected_calibration_error(y_true, output_mean_MC)
        print_and_log('* Sequential Ensemble Model over MC samples for a given checkpoint: ECE = {}'.format(indi_ece_MC))
        print_and_log('* Sequential Ensemble Model over MC samples for a given checkpoint: NLL = {}'.format(indi_nll_MC))
        val_ece_MC.append(indi_ece_MC)
        nll_loss_MC.append(indi_nll_MC)
        individual_ece_MC_mean = np.array(val_ece_MC).mean(axis=0)
        individual_ece_MC_std = np.array(val_ece_MC).std(axis=0)
        individual_nll_MC_mean = np.array(nll_loss_MC).mean(axis=0)
        individual_nll_MC_std = np.array(nll_loss_MC).std(axis=0)
        print(f"Averaged Sequential MC Ensemble Model (MC={args.num_MC_Test}): ece is {individual_ece_MC_mean} and std is {individual_ece_MC_std}")
        print(f"Averaged Sequential MC Ensemble Model (MC={args.num_MC_Test}): NLL is {individual_nll_MC_mean} and std is {individual_nll_MC_std}")
        ...
        print('* Sequential Ensemble Model Results: ECE = {}'.format(ece))
        print('* Sequential Ensemble Model Results: NLL = {}'.format(nll))

        output_mean_MC_All = np.mean(np.stack(all_folds_preds_MC, 0), 0)
        ece_MC_All = expected_calibration_error(y_true, output_mean_MC_All)
        nll_MC_All = F.nll_loss(torch.from_numpy(output_mean_MC_All).log(), torch.from_numpy(y_true), reduction="mean")
        print('* Sequential Ensemble Model Results (MC={}): ECE = {}'.format(args.num_MC_Test,ece_MC_All))
        print('* Sequential Ensemble Model Results (MC={}): NLL = {}'.format(args.num_MC_Test,nll_MC_All))
...
def main():
    ...
    parser.add_argument('--num-MC-Test', type=int, default=5, help='training epochs of exploration phase')
        ...
        if args.resume:
                ...
                if 'predict' in args.mode:
                    ...
                    for file in range(0, len(model_files)):
                        ...
                        ind_folds_preds_MC = []
                        ind_folds_preds_MC.append(current_fold_preds)
                        all_folds_preds_MC.append(current_fold_preds)
                        for i in range(num_MC_Test-1):
                            torch.manual_seed(args.seed+i+1)
                            indi_acc, indi_loss = evaluate(args, model, device, test_loader)
                            val_acc.append(indi_acc)
                            nll_loss.append(indi_loss)
                            current_fold_preds_MC, _ = evaluate_ensemble(args, model, device, test_loader)
                            ind_folds_preds_MC.append(current_fold_preds_MC)
                            all_folds_preds_MC.append(current_fold_preds_MC)
                        ...
                        output_mean_MC = torch.mean(torch.stack(ind_folds_preds_MC, dim=0), dim=0)
                        indi_loss_MC = F.nll_loss(torch.log(output_mean_MC), target, reduction='mean').item()
                        pred_MC = output_mean_MC.argmax(dim=1, keepdim=True)  # get the index of the max log-probability
                        correct_MC = pred_MC.eq(target.view_as(pred_MC)).sum().item()
                        n = target.shape[0]
                        indi_acc_MC = correct_MC / float(n)
                        print_and_log('Sequential Ensemble Model over MC samples for a given checkpoint: NLL: {:.4f}, Accuracy: {}/{} ({:.3f}%)\n'.format(
                                indi_loss_MC, correct_MC, n, 100. * correct_MC / float(n)))
                        val_acc_MC.append(indi_acc_MC)
                        nll_loss_MC.append(indi_loss_MC)
                        individual_acc_MC_mean = np.array(val_acc_MC).mean(axis=0)
                        individual_acc_MC_std = np.array(val_acc_MC).std(axis=0)
                        individual_nll_MC_mean = np.array(nll_loss_MC).mean(axis=0)
                        individual_nll_MC_std = np.array(nll_loss_MC).std(axis=0)
                        print(f"\nAveraged Sequential MC Ensemble Model (MC={num_MC_Test}): acc is {individual_acc_MC_mean} and std is {individual_acc_MC_std}")
                        print(f"Averaged Sequential MC Ensemble Model (MC={num_MC_Test}): NLL is {individual_nll_MC_mean} and std is {individual_nll_MC_std}")
                        ...
                        print('\nSequential Ensemble Model Results: NLL loss: {:.4f}, Accuracy: {}/{} ({:.3f}%)\n'.format(
                        test_loss, correct, n, 100. * correct / float(n)))

                        output_mean_MC_All = torch.mean(torch.stack(all_folds_preds_MC, dim=0), dim=0)
                        indi_loss_MC_All  = F.nll_loss(torch.log(output_mean_MC_All), target, reduction='mean').item()
                        pred_MC_All  = output_mean_MC_All .argmax(dim=1, keepdim=True)  # get the index of the max log-probability
                        correct_MC_All  = pred_MC_All .eq(target.view_as(pred_MC_All )).sum().item()

                        print('Sequential Ensemble Model Results (MC={}): NLL loss: {:.4f}, Accuracy: {}/{} ({:.3f}%)\n'.format(
                        num_MC_Test, indi_loss_MC_All, correct_MC_All, n, 100. * correct_MC_All / float(n)))
                ...
                if 'adv' in args.mode:
                    ...
                    clip_min = torch.Tensor(clip_min).reshape([channel, 1, 1]).to(device)
                    clip_max = torch.Tensor(clip_max).reshape([channel, 1, 1]).to(device)
                    ...
                    for file in range(0, len(model_files)):
                        ...
                        indi_acc, indi_loss = evaluate(args, model, device, test_loader)
                        val_acc.append(indi_acc)
                        val_nll_loss.append(indi_loss)

                        individual_acc_mean = np.array(val_acc).mean(axis=0)
                        individual_nll_mean = np.array(val_nll_loss).mean(axis=0)
                        print(f"Individual model: acc is {indi_acc}")
                        print(f"Individual model: NLL is {indi_loss}")
                        print(f"Averaged individual model: acc is {individual_acc_mean}")
                        print(f"Averaged individual model: NLL is {individual_nll_mean}")

                        print("\nData length = {}, Number of batches = {}".format(len(test_loader.dataset),len(test_loader)))
                        indi_acc, indi_acc_adv, indi_loss, indi_loss_adv = adv_test(args,model, device, test_loader,clip_min,clip_max,epsilon=args.epsilon)
                        acc.append(indi_acc)
                        nll_loss.append(indi_loss)
                        acc_adv.append(indi_acc_adv)
                        nll_loss_adv.append(indi_loss_adv)
                        print(f"Individual model: acc is {indi_acc} and adv acc is {indi_acc_adv}")
                        print(f"Individual model: NLL is {indi_loss} and adv NLL is {indi_loss_adv}")

                        individual_acc_mean = np.array(acc).mean(axis=0)
                        individual_acc_adv_mean = np.array(acc_adv).mean(axis=0)
                        individual_nll_mean = np.array(nll_loss).mean(axis=0)
                        individual_nll_adv_mean = np.array(nll_loss_adv).mean(axis=0)
                        print(f"Averaged individual model: acc is {individual_acc_mean} and adv acc is {individual_acc_adv_mean}")
                        print(f"Averaged individual model: NLL is {individual_nll_mean} and adv NLL is {individual_nll_adv_mean}")
                ###############################################################################
                #                          adversarial                                        #
                ###############################################################################
                if 'ood' in args.mode:
                        ...
                        if args.ood_dataset == 'svhn':
                            test_loader = torch.utils.data.DataLoader(
                                    make_ood_dataset(
                                        ...
                                        SVHN_M('./_dataset', train=False, download=True, transform=test_transform, in_dist_label=0)
                                    ),
                ...
                if 'tsne' == args.mode:
                    ...
                    plt.savefig('RN32_CF10_dense_.pdf')
                    # reshape
